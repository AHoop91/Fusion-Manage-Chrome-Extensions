...
import { renderItemSelectorSearchPhase } from '../view'
...
describe('item selector view', () => {
  it('renders the basic search state and result counts', () => {
    const container = document.createElement('div')
    const handlers = createHandlers()

    renderItemSelectorSearchPhase(container, {
      advancedMode: false,
  ...
  it('renders advanced mode groups and wires filter interactions', () => {
    ...
    groupLogic.value = '(A OR B)'
    groupLogic.dispatchEvent(new Event('input'))

    const inputs = Array.from(container.querySelectorAll('input.plm-extension-bom-clone-input')) as HTMLInputElement[]
    const valueInput = inputs.find((input) => input.value === 'Widget') as HTMLInputElement
    valueInput.value = 'Updated'
    valueInput.dispatchEvent(new Event('input'))

    ;(container.querySelector('.plm-extension-bom-clone-group-pill-remove') as HTMLButtonElement).click()
  ...
  it('loads more on scroll and closes the details pane from the header button', () => {
    ...
    const resultsBody = container.querySelector('.plm-extension-bom-clone-results-body') as HTMLDivElement
    ...
    resultsBody.dispatchEvent(new Event('scroll'))

    ;(container.querySelector('.plm-extension-bom-clone-details-close') as HTMLButtonElement).click()
