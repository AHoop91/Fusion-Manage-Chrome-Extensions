export type BomCloneShellLayout = 'search' | 'structure'

/**
 * Shared BOM clone modal shell (header, footer, mode toggle, group joins).
 * Search dialog field/result styles stay in searchDialogModal.styles.ts.
 */
export function buildBomCloneShellStyles(modalId: string, layout: BomCloneShellLayout = 'search'): string {
  const structureShell =
    layout === 'structure'
      ? `
...
#${modalId} .plm-extension-bom-clone-footer-actions {
  margin-left: auto;
  display: inline-flex;
  align-items: center;
