...
import { fetchWorkspaceFeatureTier, fetchWorkspaceTierSnapshot, isDesignWorkspaceSystemName } from '../workspaceFeatureTier'
...
describe('fetchWorkspaceFeatureTier', () => {
  ...
  it('returns design when API returns CW_DRAWINGS', async () => {
    ...
    await expect(fetchWorkspaceFeatureTier(runtime, 'TEST', 58)).resolves.toBe('design')
  })
  ...
  it('returns professional for any other systemName', async () => {
    ...
    await expect(fetchWorkspaceFeatureTier(runtime, 'TEST', 59)).resolves.toBe('professional')
  })
  ...
  it('returns professional on API failure', async () => {
    ...
    await expect(fetchWorkspaceFeatureTier(runtime, 'TEST', 1)).resolves.toBe('professional')
  })
})
