...
/** Matches: `https://TENANT.autodeskplm360.net/...` */
const TENANT_HOST_RE = /^(?:https?:\/\/)?(?<tenant>[^.]+)\.autodeskplm360\.net/i

/** Matches: `/plm/workspaces/86` */
 ...
 *
 * Example: `urn`adsk.plm`tenant,workspace,item`TENANT_NAME,86,11710`
 */
function parseItemIdParam(itemIdParam: string | null): { workspaceId: number | null; dmsId: number | null } {
...
/** Extract tenant, workspaceId, item dmsId, and item tab (if present) from a Fusion Manage URL. */
export function getFusionContextFromUrl(urlString: string): FusionContext {
  let url: URL
  ...
  const workspaceFromPath = parseIntOrNull(WORKSPACE_PATH_RE.exec(url.pathname)?.groups?.workspaceId)

  const { workspaceId: workspaceFromParam, dmsId } = parseItemIdParam(url.searchParams.get('itemId'))
