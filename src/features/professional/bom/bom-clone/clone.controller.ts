...
import { createCloneDom, type CloneDomAdapter } from './bom-clone-dom'
...
export function createCloneController(runtime: CloneRuntime): CloneController {
  ...
  function syncDom(): void {
    ...
    if (!features.enableBomVariant && !features.enableBomManufacturing) {
      dom.removeCloneButton()
      closeModal()
      return
    }
    ...
    const button = dom.ensureCloneButton((mode) => {
      if (!isCloneModeEnabled(mode)) return
      void (async () => {
        state.setPermissionsLoading(true)
      ...
      disabled: false,
      title: 'Quick Create',
      showVariantBom: features.enableBomVariant,
      showManufacturingBom: features.enableBomManufacturing
    })
    if (!button) {
      return
    }
  ...
  function isCloneModeEnabled(mode: CloneLaunchMode): boolean {
    const features = getEffectiveFeatures()
    ...
    mount() {
      ...
      if (keepAliveTimer === null) {
        keepAliveTimer = window.setInterval(() => {
          ...
          const features = getEffectiveFeatures()
          if (!features.enableBomVariant && !features.enableBomManufacturing) return
