...
export type CloneDomAdapter = {
  ...
  ensureCloneButton: (
    ...
    options?: {
      disabled?: boolean
      title?: string
      /** When false, hide “Variant Bill of Materials” (engineering). Default true. */
      showVariantBom?: boolean
      /** When false, hide “Manufacturing Bill of Materials”. Default true. */
      showManufacturingBom?: boolean
    }
  ) => HTMLDivElement | null
...
function ensureStyles(): void {
  ensureSearchDialogModalStyles(SEARCH_MODAL_ID)
...
export function createCloneDom(runtime: BomCloneDomRuntime): CloneDomAdapter {
  ...
  return {
    ...
    ensureCloneButton(onSelect, options) {
      ...
      const showVariant = options?.showVariantBom !== false
      const showMfg = options?.showManufacturingBom !== false
      if (!showVariant && !showMfg) {
        const existing = document.getElementById(CLONE_DROPDOWN_ID)
      ...
      engineering.textContent = 'Variant Bill of Materials'
      manufacturing.textContent = 'Manufacturing Bill of Materials'
      engineering.hidden = !showVariant
      manufacturing.hidden = !showMfg
