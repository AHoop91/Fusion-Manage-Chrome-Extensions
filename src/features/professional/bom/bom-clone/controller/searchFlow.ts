...
import type { CloneControllerRefs, CloneRuntime, CloneState, CloneViewHandlers, ItemSelectorSession } from './types'
import type { CloneDomAdapter } from '../bom-clone-dom'
...
export function createCloneSearchFlow(options: SearchFlowOptions): {
  ...
  async function validateAndLoadStructure(): Promise<void> {
    ...
    try {
      ...
      try {
        await service.validateLinkableItem(activeContext, sourceItemId)
      } catch {
        // Non-blocking: continue loading BOM even when validation fails.
      }
