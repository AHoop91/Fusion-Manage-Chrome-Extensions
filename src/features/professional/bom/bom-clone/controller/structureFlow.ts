...
import type { CloneControllerRefs, CloneState, CloneViewHandlers } from './types'
      ...
      if (tree === 'source') {
        ...
        if (sourceRootNodeId) {
          ...
          try {
            ...
            // Ignore expand failures; UI loading flags still reset in outer flow.
          }
        }
      ...
      onDropSourceAssemblySubcomponentsToTarget(nodeId: string) {
        void (async () => {
          ...
          try {
            ...
            // Ignore subtree load failure for drag staging.
          } finally {
