...
import { resolveBomPageContext } from '../bom-shared/page'
import { createCloneDom } from './bom-clone-dom'
...
export function createBomCloneFeature(runtime: CloneRuntime): BomCloneFeature {
  ...
  async function openClone(mode: CloneLaunchMode): Promise<void> {
    ...
    if (mode === 'engineering' && !f.enableBomVariant) return
    if (mode === 'manufacturing' && !f.enableBomManufacturing) return
  ...
  function syncLauncher(): void {
    ...
    dom.ensureCloneButton((mode) => {
      ...
      disabled: false,
      title: 'Quick Create',
      showVariantBom: f.enableBomVariant,
      showManufacturingBom: f.enableBomManufacturing
    })
  }
    ...
    mount() {
      ...
      if (keepAliveTimer === null) {
        keepAliveTimer = window.setInterval(() => {
          ...
          const flags = getEffectiveFeatures()
          if (!flags.enableBomVariant && !flags.enableBomManufacturing) return
