...
import { attachNumberInputWheelCapture } from '../../../../../../../shared/form/numberInputWheel'
import {
  isBooleanFieldType,
  isDateFieldType,
  isDecimalFieldType,
  ...
  if (field.formulaField) {
    ...
    control = (
      <input
        ...
        inputMode="decimal"
        {...(!isIntegerFieldType(field.typeId) && numericPrecision > 0
          ? { 'data-plm-wheel-step': String(Math.pow(10, -numericPrecision)) }
          : {})}
        onChange={(event) => onChange({ value: event.target.value, displayValue: event.target.value })}
        ref={(node) => {
          if (node) attachNumberInputWheelCapture(node)
        }}
      />
    )
