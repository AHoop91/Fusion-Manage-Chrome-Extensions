...
async function fetchJson<T>(requestUrl: string): Promise<T | null> {
  ...
  if (!tenant) {
    ...
    setGridFieldHydrationIssue(message)
    warnGridDiagnostic('tenant-missing', message)
    return null
  }
  try {
    ...
    const message = `Fusion API request failed (${detail}).`
    setGridFieldHydrationIssue(message)
    warnGridDiagnostic(`fetch-${requestUrl}`, message, error)
    return null
  }
}
...
export async function hydrateGridFieldsForCurrentContext(): Promise<boolean> {
  ...
  if (!context) {
    const message =
      'Grid URL is missing required query params (tab=grid, view=full|split, itemId, and view mode).'
    setGridFieldHydrationIssue(message)
    warnGridDiagnostic('grid-context', message)
    return false
  }
    ...
    try {
      for (const viewId of getGridViewIdCandidates(context.workspaceId, context.dmsId)) {
        ...
        if (Array.isArray(cached?.fields) && cached.fields.length > 0) {
          setGridFieldHydrationIssue(null)
          return true
        }
      ...
      const message = 'Fusion returned no grid field metadata for this item (check view id / permissions).'
      setGridFieldHydrationIssue(message)
      warnGridDiagnostic('hydrate-empty', message)
      return false
    } finally {
