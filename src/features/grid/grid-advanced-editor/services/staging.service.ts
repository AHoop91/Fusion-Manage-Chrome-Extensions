...
export interface StagingManager {
  ...
  /** Copy a staged new row into a new insert draft (for Clone on unsaved rows). */
  buildCloneDraftFromInsertAt: (insertIndex: number, columns: ApiTableColumn[]) => PendingInsertDraftLike
  ...
  /** Clears all staged insert drafts (New count). */
  clearStagedNewRows: () => void
  /** Clears all staged field edits on existing rows (Edited count). */
  clearStagedEdits: () => void
  /** Clears all staged deletes on existing rows (Deleted count). */
  clearStagedDeletes: () => void
...
export function createStagingManager(deps: StagingManagerDeps): StagingManager {
  ...
  return {
    ...
    buildCloneDraftFromModel: (model, columns) => {
      ...
      for (const fieldId of deps.gridService.getUniqueInGridFieldIdsForCurrentGrid()) {
        payload.set(fieldId, '')
        display.set(fieldId, '')
      }
      return { payload, display, source: 'clone' }
    },
    buildCloneDraftFromInsertAt: (insertIndex, columns) => {
      const source = pendingInsertDrafts[insertIndex]
      const payload = new Map<string, string>()
      const display = new Map<string, string>()
      if (source) {
        for (const column of columns) {
          if (!column.field.editable) continue
          const id = column.field.fieldId
          payload.set(id, source.payload.get(id) ?? '')
          display.set(id, source.display.get(id) ?? column.field.defaultValue ?? '')
        }
      }
      for (const fieldId of deps.gridService.getUniqueInGridFieldIdsForCurrentGrid()) {
        payload.set(fieldId, '')
    ...
    clearStagedNewRows: () => queue.clearInsertsOnly(),
    clearStagedEdits: () => queue.clearUpdatesOnly(),
    clearStagedDeletes: () => queue.clearRemovalsOnly(),
