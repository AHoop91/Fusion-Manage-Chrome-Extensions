...
export interface GridService {
  ...
  getUniqueInGridFieldIdsForCurrentGrid: () => string[]
...
export function createGridService(): GridService {
  ...
  return {
    ...
    hydrateRequiredValidatorsForFields: metadataCache.hydrateRequiredValidatorsForFields,
    ensureValidatorsHydratedForCurrentGrid: async () => {
      const payload = repository.getGridFieldsPayloadForCurrentGrid()
      if (payload?.fields?.length) {
        await metadataCache.hydrateRequiredValidatorsForFields(payload.fields)
      }
    }
  }
}
