...
/**
 * Row/column coordinates for a duplicate cell when it maps to the modal row table.
 */
export type UniqueInGridDuplicateCell =
  | { kind: 'existing'; domRowIndex: number }
  | { kind: 'insert'; insertIndex: number }

/**
 * Non-unique value for a field that has a `uniqueInGrid` validator.
 */
export interface UniqueInGridValidationIssue {
  rowLabel: string
  fieldTitle: string
  duplicateValue: string
  fieldId: string
  /** When set, the row table can highlight this cell (later duplicate rows only). */
  duplicateCell?: UniqueInGridDuplicateCell
}
...
export interface ValidationManager {
  ...
  getUniqueInGridValidationIssues: (modelByDomRowIndex: Map<number, SelectedRowModel>) => UniqueInGridValidationIssue[]
  getUniqueDuplicateFieldTitles: (issues: UniqueInGridValidationIssue[]) => string[]
}
...
export function createValidationManager(deps: ValidationManagerDeps): ValidationManager {
  ...
  function getUniqueDuplicateFieldTitles(issues: UniqueInGridValidationIssue[]): string[] {
    const unique = new Set<string>()
    for (const issue of issues) {
      const normalized = issue.fieldTitle.trim()
      if (!normalized) continue
  ...
  function getUniqueInGridValidationIssues(
    modelByDomRowIndex: Map<number, SelectedRowModel>
  ): UniqueInGridValidationIssue[] {
    const uniqueFieldIds = deps.gridService.getUniqueInGridFieldIdsForCurrentGrid()
    ...
    const columnByFieldId = new Map(metadata.apiTableColumns.map((column) => [column.field.fieldId, column]))

    const normalizeUniqueValue = (value: string): string => String(value || '').trim()
    ...
    const issues: UniqueInGridValidationIssue[] = []
    const seen = new Set<string>()
    const pushIssue = (
      rowLabel: string,
      fieldTitle: string,
      duplicateValue: string,
      fieldIdForIssue: string,
      duplicateCell?: UniqueInGridDuplicateCell
    ): void => {
      const key = `${rowLabel}::${fieldTitle}::${duplicateValue}`
      if (seen.has(key)) return
      seen.add(key)
      issues.push({ rowLabel, fieldTitle, duplicateValue, fieldId: fieldIdForIssue, duplicateCell })
    }

    for (const fieldId of uniqueFieldIds) {
      const column = columnByFieldId.get(fieldId)
      if (!column) continue
      const fieldTitle = column.field.title
      ...
      const resolveEffectiveForColumn = (
        fieldIdInner: string,
        domRowIndex: number | null,
        rowModel: SelectedRowModel | null,
        rowChanges: Map<string, string> | null | undefined,
        apiRow: (typeof metadata.apiRows)[number] | null
      ): string => {
        const pendingDisplay = domRowIndex !== null ? pendingDisplayByDomRowIndex.get(domRowIndex)?.get(fieldIdInner) : undefined
        if (typeof pendingDisplay === 'string') return pendingDisplay
        if (rowChanges?.has(fieldIdInner)) return String(rowChanges.get(fieldIdInner) ?? '')
        if (rowModel) return deps.gridService.getApiTableValueForRow(rowModel, column, pendingChangesByDomRowIndex)
        if (apiRow) return apiRow.byFieldId.get(fieldIdInner) || ''
        return ''
      }
      ...
      if (metadata.apiRows.length > 0) {
        for (const apiRow of metadata.apiRows) {
          ...
          const rowModel = domRowIndex !== null ? modelByDomRowIndex.get(domRowIndex) || null : null
          const rowChanges = domRowIndex !== null ? pendingChangesByDomRowIndex.get(domRowIndex) : null
          const rowLabel = `Row ${apiRow.rowId || String(apiRow.index + 1)}`
          entries.push({
            rowLabel,
            value: resolveEffectiveForColumn(fieldId, domRowIndex, rowModel, rowChanges, apiRow),
            duplicateCell:
              domRowIndex !== null && Number.isFinite(domRowIndex)
                ? { kind: 'existing' as const, domRowIndex }
                : undefined
          })
        }
      } else {
        ...
        for (const domRowIndex of existingRowIndexesToValidate) {
          ...
          const rowChanges = pendingChangesByDomRowIndex.get(domRowIndex) || new Map<string, string>()
          entries.push({
            rowLabel: `Row ${domRowIndex + 1}`,
            value: resolveEffectiveForColumn(fieldId, domRowIndex, model, rowChanges, model.apiRow),
            duplicateCell: { kind: 'existing', domRowIndex }
          })
        }
      }
      ...
      for (let insertIndex = 0; insertIndex < pendingInsertDrafts.length; insertIndex += 1) {
        ...
        entries.push({
          ...
          value:
            draft.display.get(fieldId) ||
            draft.payload.get(fieldId) ||
            column.field.defaultValue ||
            '',
          duplicateCell: { kind: 'insert', insertIndex }
        })
      }

      const firstRowByNormalized = new Map<string, string>()
      for (const entry of entries) {
        const normalized = normalizeUniqueValue(entry.value)
        if (!normalized) continue
        if (!firstRowByNormalized.has(normalized)) {
          firstRowByNormalized.set(normalized, entry.rowLabel)
          continue
        }
        pushIssue(entry.rowLabel, fieldTitle, normalized, fieldId, entry.duplicateCell)
      }
    }
  ...
  return {
    ...
    getUniqueInGridValidationIssues,
    getUniqueDuplicateFieldTitles
  }
}
