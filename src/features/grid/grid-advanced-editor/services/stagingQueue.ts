...
export interface StagingQueue {
  ...
  clear: () => void
  /**
   * Clears staged inserts only.
   */
  clearInsertsOnly: () => void
  /**
   * Clears staged row updates only (payload/display maps).
   */
  clearUpdatesOnly: () => void
  /**
   * Clears staged removals only.
   */
  clearRemovalsOnly: () => void
}
...
export function createStagingQueue(): InternalStagingQueue {
  ...
  function clearInsertsOnly(): void {
    inserts.length = 0
  }

  function clearUpdatesOnly(): void {
    updatesPayloadByRow.clear()
    updatesDisplayByRow.clear()
  }

  function clearRemovalsOnly(): void {
    removals.clear()
  }
