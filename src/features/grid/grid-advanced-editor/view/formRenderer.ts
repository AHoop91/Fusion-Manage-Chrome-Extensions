...
export function createFormRenderer(gridService: GridService): FormRenderer {
  ...
  function resolveLookupPayloadValue(control: HTMLInputElement, userInputValue: string, isMultiSelect: boolean): string {
    ...
    const linkFromLabelMap = currentLabel ? labelToLink.get(normalizeText(currentLabel)) || '' : ''
    const effectiveLink = currentLink || linkFromLabelMap

    if (currentLabel && normalizedInput && currentLabel.toLowerCase() === normalizedInput.toLowerCase() && effectiveLink) {
      return effectiveLink
    }

    if (effectiveLink && !normalizedInput) return effectiveLink
