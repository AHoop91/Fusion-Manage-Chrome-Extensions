...
import { buildSummaryNode, type StagedSummary, type StagedSummaryClearHandlers } from './summaryView'
...
export interface RequiredSummaryRenderState {
  ...
  /** Column titles for fields that violate unique-in-grid rules */
  uniqueDuplicateFields: string[]
}
...
export interface RenderRowTableArgs {
  ...
  /** Cells with duplicate values for unique-in-grid columns (see row table renderer key format). */
...
export interface ViewRenderer {
  ...
  setStatus: (summary: StagedSummary, extra?: string, clearHandlers?: StagedSummaryClearHandlers) => void
  ...
  setImportFileActionVisible: (visible: boolean) => void
...
export function createViewRenderer(refs: ModalDomRefs): ViewRenderer {
  ...
  function setStatus(summary: StagedSummary, extra = '', clearHandlers?: StagedSummaryClearHandlers): void {
    refs.status.textContent = ''
    refs.status.appendChild(buildSummaryNode(summary, 'plm-extension-grid-form-status-summary', clearHandlers))
    if (extra) {
  ...
  function renderRequiredSummary(state: RequiredSummaryRenderState): void {
    ...
    if (!state.canEvaluateRequired) {
      requiredSummary.appendChild(
        el('span').cls('plm-extension-grid-form-required-summary-label').text('Required Fields Remaining:').build()
      )
    ...
    if (state.requiredFields.length === 0 && state.uniqueDuplicateFields.length === 0) {
      requiredSummary.appendChild(
        el('span').cls('plm-extension-grid-form-required-summary-label').text('Required Fields Remaining:').build()
      )
    ...
    if (state.requiredFields.length > 0) {
      ...
      for (const field of state.requiredFields) {
        pills.appendChild(el('span').cls('plm-extension-grid-form-required-pill').text(field).build())
      }
      requiredSummary.appendChild(pills)
    }
    if (state.uniqueDuplicateFields.length > 0) {
      requiredSummary.appendChild(
        el('span').cls('plm-extension-grid-form-required-summary-label').text('Must be unique:').build()
      )
      const uniquePills = el('span').cls('plm-extension-grid-form-required-pill-list').build()
      for (const field of state.uniqueDuplicateFields) {
        uniquePills.appendChild(el('span').cls('plm-extension-grid-form-required-pill').text(field).build())
      }
      requiredSummary.appendChild(uniquePills)
    }
  ...
  function updateCommitActionState(state: CommitActionRenderState): void {
    ...
    const hasUniqueIssues = state.uniqueDuplicateFields.length > 0
    ...
    const saveDisabled =
      baseDisabled || !state.hasPendingOperations || hasRequiredIssues || hasUniqueIssues || hasUnauthorizedStagedOperations
    commitAction.disabled = saveDisabled
    commitAndCloseAction.disabled = saveDisabled
    if (baseDisabled) {
      commitAction.title = ''
      commitAndCloseAction.title = ''
    } else if (hasRequiredIssues) {
      const title = `Pending required attributes: ${state.requiredFields.join(', ')}`
      commitAction.title = title
      commitAndCloseAction.title = title
    } else if (hasUniqueIssues) {
      const title = `Duplicate values (must be unique): ${state.uniqueDuplicateFields.join(', ')}`
      commitAction.title = title
      commitAndCloseAction.title = title
    } else if (hasUnauthorizedAdds) {
      const title = 'Missing permission: Add to Grid'
      commitAction.title = title
      commitAndCloseAction.title = title
    } else if (hasUnauthorizedUpdates) {
      const title = 'Missing permission: Edit Grid'
      commitAction.title = title
      commitAndCloseAction.title = title
    } else if (hasUnauthorizedDeletes) {
      const title = 'Missing permission: Delete from Grid'
      commitAction.title = title
      commitAndCloseAction.title = title
    } else if (!state.hasPendingOperations) {
      const title = 'No staged changes to save'
      commitAction.title = title
      commitAndCloseAction.title = title
    } else {
      commitAction.title = 'Save staged changes and keep the editor open'
      commitAndCloseAction.title = 'Save staged changes and close the editor'
    }
  }

  function setImportFileActionVisible(visible: boolean): void {
    importFileAction.style.display = visible ? '' : 'none'
  }
  ...
  function syncEditActionState(state: EditActionRenderState): void {
    ...
    importFileAction.disabled = state.isCommitting || !state.hasApiFieldMetadata
    importFileAction.title = importFileAction.disabled
      ? state.isCommitting
        ? 'Import is unavailable while saving'
        : 'Import is unavailable until grid metadata has loaded'
      : 'Import CSV rows into this grid'
  }
  ...
  function renderRowTable(args: RenderRowTableArgs): void {
    ...
    renderModalRowTable({
      ...
      uniqueDuplicateCellKeys: args.uniqueDuplicateCellKeys ?? new Set<string>()
    })
  }
