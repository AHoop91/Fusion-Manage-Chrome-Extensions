...
function picklistValueUrnFromRaw(fieldId: string, activeModel: SelectedRowModel | null): string {
  const raw = activeModel?.apiRow?.rawByFieldId.get(fieldId)
  if (!raw || !Object.prototype.hasOwnProperty.call(raw, 'value')) return ''
  const v = raw.value
  if (v == null || typeof v !== 'object' || Array.isArray(v)) return ''
  const urn = String((v as Record<string, unknown>).urn || '').trim()
  return urn.toLowerCase().startsWith('urn:') ? urn : ''
}
...
export function renderFormFields(args: FormFieldsRenderArgs): RowBinding[] {
  ...
  for (const matchedField of matchedFields) {
    ...
    if (fieldUsesLookup && control instanceof HTMLInputElement) {
      ...
      const urnFromRaw = picklistValueUrnFromRaw(field.fieldId, activeModel)
      if (urnFromRaw) control.dataset.plmLookupCurrentUrn = urnFromRaw
      else delete control.dataset.plmLookupCurrentUrn
      ...
      control.dataset.plmLookupDefaultLabel = defaultLabel
      control.dataset.plmLookupDefaultLink = defaultLink
      const urnFromRawRadio = picklistValueUrnFromRaw(field.fieldId, activeModel)
      if (urnFromRawRadio) control.dataset.plmLookupCurrentUrn = urnFromRawRadio
      else delete control.dataset.plmLookupCurrentUrn
    }
