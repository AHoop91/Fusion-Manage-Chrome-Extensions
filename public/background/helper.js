...
const lastBadgeModeByTab = new Map();
...
function getDiagnosticSignatureCandidates(urlString) {
    try {
        ...
        if (pathname.includes('/items/bom/nested') && tab === 'bom') return ['bom.clone', 'runtime-baseline:bom'];
...
function getExpectedFeatureKeys(urlString) {
    try {
        ...
        if (pathname.includes('/items/bom/nested') && tab === 'bom') return ['bom.clone'];
...
function resolveBadgeMode(diagnostic, expectedFeatureKeys) {
...
function getStorageValues(keys, area = 'local') {
...
async function resolveDiagnosticForUrl(urlString) {
...
async function setBadgeMode(tabId, mode) {
...
/**
 * Updates extension badge + icon based on whether the tab
 * is a Fusion Manage page and current health diagnostics.
 */
export async function updateActionForTab(tabId, url, diagnosticOverride = null) {
    if (!url) return;
